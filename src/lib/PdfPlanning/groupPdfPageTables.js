...
export function groupPdfPageTables(pageTables) {
  if (!pageTables || !pageTables.length) {
    throw new Error('no-table', 'Planning introuvable.')
  }

  const pages = _.chain(pageTables)
    .map(table => {
      console.log([...table])
      const meta = {}
      let line = table.shift()
      let found
      while (line) {
        if (line.join(';') === HEADER) {
          found = true
      ...
      if (!found) {
        console.log('%cAucun tableau trouvé ou format inconnu !', 'color:red', table)
        return null
      }
  ...
  if (!checkFormat) {
    throw new Error('format-error', 'Format de fichier incorrect !')
  }
